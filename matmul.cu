...
void matMulCPU(float* A, float* B, float* C, int numARows, int numACols,
               int numBCols) {
    int i, j, k;
    int offsetA, offsetB;
    float cumSum;
...
__global__ void matMulGPU(float* A, float* B, float* C, int numARows,
                          int numACols, int numBCols) {
...
int main(void) {
    ...
    dim3 blockPerGrid(ceil(numBCols / (float)BLOCK_SIZE),
                      ceil(numACols / (float)BLOCK_SIZE), 1);
    timer2.Start();
    matMulGPU<<<blockPerGrid, threadPerBlock>>>(d_A, d_B, d_C, numARows, numACols,
                                                numBCols);
    timer2.Stop();
    ...
    timer3.Stop();
    printf(
        "Time to copy the resulting Matrix from device to host is: %f msecs.\n",
        timer3.Elapsed());
    ...
    for (i = 0; i < numACols; i++)
        for (j = 0; j < numBCols; j++)
            if (fabs(h_C_CPU[i * numBCols + j] - h_C[i * numBCols + j]) > 1e-2) {
                fprintf(stderr,
                        "Result verification failed at element (%d,%d)!\n", i, j);
                exit(EXIT_FAILURE);
            }
    printf("Test PASSED\n");
